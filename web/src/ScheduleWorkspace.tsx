...
function checks(groups: Group[], selected: string[], setSelected: (ids: string[]) => void) { return <div className="checks fluent-checks">{groups.map((group) => <Checkbox id={`schedule-group-${group.id}`} key={group.id} checked={selected.includes(group.id)} onChange={(_, data) => setSelected(data.checked ? [...selected, group.id] : selected.filter((id) => id !== group.id))} label={group.name} />)}</div> }
...
function assignments(schedule: Schedule): Assignment[] { return schedule.days.flatMap((day) => day.entries.filter((entry) => entry.type === 'class' || entry.type === 'activity').map((entry) => ({ id: `${day.id}:${entry.id}`, timelineId: day.timelineId ?? day.id, entryId: entry.sourceEntryId ?? entry.id, dayOfWeek: day.dayOfWeek, weeks: day.weeks, subjectId: entry.subjectId }))) }
...
function SchedulePublishLibrary({ records, groups, onPublish }: { records: ScheduleRecord[]; groups: Group[]; onPublish: (record: ScheduleRecord, ids: string[]) => void }) { const [selection, setSelection] = useState<Record<string, string[]>>({}); return <section className="publish-library data-section"><div className="section-heading"><h2>独立发布</h2><span>替换已保存课表的发布目标</span></div>{records.map((record) => { const ids = selection[record.id] ?? record.group_ids; return <article key={record.id}><div><strong>{record.name}</strong><span>r{record.revision}</span></div>{checks(groups, ids, (value) => setSelection({ ...selection, [record.id]: value }))}<button className="primary" onClick={() => onPublish(record, ids)}><Send24Regular />{ids.length ? '发布' : '取消发布'}</button></article>})}</section> }
