...
export function ScheduleWorkspace({ organizationId, groups, classGroups, onComplete }: Props) {
  ...
  async function load() { if (!organizationId) return; try { const [nextRecords, nextTimelines, nextCourses] = await Promise.all([api.schedules(organizationId), api.timelines(organizationId), api.courses(organizationId)]); setRecords(nextRecords); setTimelines(nextTimelines); setCourses(nextCourses.filter((course) => course.isLocalClassroom)) } catch { /* parent handles authentication failures */ } }
...
function WeeklyEditor({ schedule, setSchedule, week, setWeek }: { schedule: Schedule; setSchedule: (value: Schedule) => void; week: number; setWeek: (value: number) => void }) {
  ...
  return <div className="weekly-editor"><div className="week-toolbar"><Button appearance="subtle" disabled={week <= 1} onClick={() => setWeek(week - 1)}>上一周</Button><Text weight="semibold">循环第 {week} 周</Text><Button appearance="subtle" disabled={week >= schedule.meta.maxWeekCycle} onClick={() => setWeek(week + 1)}>下一周</Button><span></span></div>{!schedule.timelineIds.length ? <div className="empty-command">请先选择时间线。</div> : <div className="weekly-grid" style={{ gridTemplateColumns: `minmax(110px, 140px) repeat(${days.length || 1}, minmax(180px, 1fr))` }}><div className="grid-head">课次</div>{days.map((day) => <div className="grid-head" key={day.id}>{DAYS[day.dayOfWeek?.[0] ? day.dayOfWeek[0] - 1 : 0]}</div>)}{Array.from({ length: rows }, (_, row) => <div className="weekly-row" key={`row-${row}`}><div className="time-cell"><Text weight="semibold">第 {row + 1} 节</Text><span>{days.find((day) => day.entries[row])?.entries[row]?.startTime ?? '--:--'}–{days.find((day) => day.entries[row])?.entries[row]?.endTime ?? '--:--'}</span></div>{days.map((day) => { const entry = day.entries[row]; return entry ? <Select key={day.id} aria-label={`${DAYS[day.dayOfWeek?.[0] ? day.dayOfWeek[0] - 1 : 0]} 第 ${row + 1} 节`} value={entry.subjectId ?? ''} onChange={(_, data) => setCell(day, entry, data.value)}><option value="">未设置</option>{schedule.subjects.map((course) => <option key={course.id} value={course.id}>{course.name}</option>)}</Select> : <div className="schedule-empty-cell" key={day.id}>无时间段</div> })}</div>)}</div>}</div>
}

function SchedulePublishLibrary({ records, groups, classGroups, onPublish }: { records: ScheduleRecord[]; groups: Group[]; classGroups: ClassGroup[]; onPublish: (record: ScheduleRecord, ids: string[]) => void }) { const [selection, setSelection] = useState<Record<string, string[]>>({}); return <section className="publish-library data-section"><div className="section-heading"><h2>独立发布</h2><span>替换已保存课表的发布目标</span></div>{records.map((record) => { const ids = selection[record.id] ?? record.group_ids; return <article key={record.id}><div><Text weight="semibold">{record.name}</Text><Text size={200}>r{record.revision}</Text></div>{checks(groups, classGroups, ids, (value) => setSelection({ ...selection, [record.id]: value }), `schedule-publish-${record.id}`)}<Button appearance="primary" icon={<Send24Regular />} onClick={() => onPublish(record, ids)}>{ids.length ? '发布' : '取消发布'}</Button></article>})}</section> }
