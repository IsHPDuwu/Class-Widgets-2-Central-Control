...
export function ClassSwapWorkspace({ organizationId, groups, devices, onComplete }: Props) {
  ...
  return <div className="workspace-stack">
    ...
    {schedule && <section className="panel"><div className="panel-heading"><div><h2>创建换课事件</h2><p>事件只会下发到 {selectedDevice?.name ?? '当前设备'}。</p></div></div>
      ...
      {entries.length > 0 ? <div className="schedule-grid">{entries.map((entry) => <div className="schedule-entry" key={entry.id}><strong>{subjectName(entry)}</strong><span>{entry.startTime}–{entry.endTime}</span><small>{entry.id}</small></div>)}</div> : <div className="empty-command">该设备在所选星期和周期周没有课程，无法创建换课事件。</div>}
      ...
      <Button appearance="primary" icon={<Send24Regular />} disabled={!preparation?.ready || entries.length === 0 || (operation === 'swap' && (!entryA || !entryB || entryA === entryB)) || (operation === 'replace' && (!entryA || !subjectId))} onClick={() => void submit()}>下发到该设备</Button>
    </section>}
