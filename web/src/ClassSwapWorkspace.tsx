...
export function ClassSwapWorkspace({ organizationId, groups, devices, onComplete }: Props) {
  ...
  async function prepare() {
    try {
      const result = await api.prepareClassSwap(groupId)
      setRequestId(result.request_id); setPreparation(null); setSchedule(null)
      onComplete('已请求客户端上传当前课表，请等待同步')
  ...
  async function refreshPreparation(id = requestId) {
    ...
    try {
      const result = await api.classSwapPreparation(id, groupId)
      setPreparation(result)
      const ready = result.devices.find((device) => device.ready)
      if (ready) {
        const snapshot = await api.classSwapSnapshot(ready.device_id, id)
        setSchedule(snapshot.schedule as unknown as Schedule)
        setWeek((current) => Math.min(current, (snapshot.schedule as unknown as Schedule).meta.maxWeekCycle))
      }
    } catch (error) { onComplete(error instanceof Error ? error.message : '刷新上传状态失败', 'error') }
  }
  async function submit() {
    const readyIds = preparation?.devices.filter((device) => device.ready).map((device) => device.device_id) ?? []
    if (!requestId || !readyIds.length) return
    try {
      await api.createClassSwap({ group_id: groupId, request_id: requestId, device_ids: readyIds, operation, day_of_week: day, week_of_cycle: week, entry_id_a: operation === 'swap' ? entryA : '', entry_id_b: operation === 'swap' ? entryB : '', entry_id: operation === 'replace' ? entryA : '', subject_id: operation === 'replace' ? subjectId : '' })
      onComplete(`已向 ${readyIds.length} 台设备下发临时换课`); loadSessions()
    } catch (error) { onComplete(error instanceof Error ? error.message : '下发失败', 'error') }
  }
  async function restore(id: string) {
    try { await api.restoreClassSwap(id); onComplete('已下发恢复课表事件'); loadSessions() }
    catch (error) { onComplete(error instanceof Error ? error.message : '恢复失败', 'error') }
  }

  const active = sessions.filter((session) => session.group_id === groupId && session.status === 'active')
  const groupDevices = devices.filter((device) => device.group_id === groupId && !device.revoked)
  return <div className="workspace-stack">
    <section className="panel"><div className="panel-heading"><div><h2>获取客户端课表</h2><p>换课前要求客户端上传当前单双周课表，事件由客户端本地执行。</p></div></div>
      <div className="form-grid"><label>设备分组<Select value={groupId} onChange={(_, data) => { setGroupId(data.value); setRequestId(''); setPreparation(null); setSchedule(null) }}>{groups.map((group) => <option key={group.id} value={group.id}>{group.name}</option>)}</Select></label></div>
      <div className="button-row"><Button appearance="primary" icon={<CalendarSync24Regular />} disabled={!groupId || !groupDevices.length} onClick={() => void prepare()}>获取客户端课表</Button><Button icon={<ArrowSync24Regular />} disabled={!requestId} onClick={() => void refreshPreparation()}>刷新上传状态</Button></div>
      {preparation && <div className="checks">{preparation.devices.map((device) => <label key={device.device_id}><span className={device.ready ? 'status-dot online' : 'status-dot'} />{device.device_name} · {device.ready ? '已上传' : '等待中'}</label>)}</div>}
    </section>
    {schedule && <section className="panel"><div className="panel-heading"><div><h2>创建换课事件</h2><p>下发目标为本次已成功上传、且 entry ID 兼容的设备。</p></div></div>
      ...
      <Button appearance="primary" icon={<Send24Regular />} disabled={!preparation?.devices.some((device) => device.ready) || (operation === 'swap' && (!entryA || !entryB || entryA === entryB)) || (operation === 'replace' && (!entryA || !subjectId))} onClick={() => void submit()}>下发临时换课</Button>
    </section>}
    {active.length > 0 && <section className="panel"><div className="panel-heading"><div><h2>今天的临时换课</h2><p>共 {active.reduce((sum, session) => sum + session.operations.length, 0)} 个操作。</p></div></div>{active.map((session) => <div className="list-row" key={session.id}><div><strong>{session.effective_date}</strong><span>{session.operations.length} 个换课操作</span></div><Button icon={<Dismiss24Regular />} onClick={() => void restore(session.id)}>立即恢复</Button></div>)}</section>}
  </div>
}
