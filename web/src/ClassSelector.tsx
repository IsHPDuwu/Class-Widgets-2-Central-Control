...
export function ClassSelector({ groups, classGroups, selected, onChange, idPrefix }: Props) {
  ...
  return <div className="class-selector">
    {classGroups.length > 0 && <Dropdown aria-label="快捷选择分组" placeholder="快捷选择分组" onOptionSelect={(_, data) => data.optionValue && addClassGroup(data.optionValue)}>{classGroups.map((item) => <Option key={item.id} value={item.id} text={item.name}>{item.name}（{item.group_ids.length} 个班级）</Option>)}</Dropdown>}
