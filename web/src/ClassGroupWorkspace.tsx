...
export function ClassGroupWorkspace({ organizationId, groups, onComplete }: Props) {
  ...
  return <div className="course-resource-workspace">
    <Card className="resource-sidebar"><CardHeader header={<strong>分组</strong>} action={<Button appearance="subtle" icon={<Add24Regular />} onClick={reset}>新建</Button>} /><div className="resource-nav">{items.map((item) => <Button className={selectedId === item.id ? 'selected' : ''} appearance="subtle" key={item.id} onClick={() => open(item)}><span><strong>{item.name}</strong><small>{item.group_ids.length} 个班级</small></span></Button>)}</div></Card>
